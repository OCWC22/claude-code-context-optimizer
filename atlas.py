...
- sandbox_computations: Vercel Sandbox execution results
...
Usage:
    ...
    # Store sandbox computation
    await atlas.store_sandbox_result(computation_id, result)
"""
...
class InMemoryCollection:
    ...
    def find(self, query: dict = None):
        # Note: Not async since it just returns a cursor
    ...
    async def delete_many(self, query: dict):
        ...
        for i in range(len(self._data) - 1, -1, -1):
            if self._matches(self._data[i], query):
                self._data.pop(i)
                deleted += 1
    ...
    async def insert_many(self, docs: list[dict]):
        for doc in docs:
            self._data.append(doc.copy())
        return type("InsertResult", (), {"inserted_ids": [doc.get("_id", str(uuid4())) for doc in docs]})()
...
class Atlas:
    ...
    async def connect(self):
        ...
        if self.uri and HAS_MOTOR:
            try:
                self._client = AsyncIOMotorClient(self.uri, serverSelectionTimeoutMS=10000)
                ...
                # Try direct connection to first shard as fallback (read-only mode)
                try:
                    # Extract host from URI and try direct connection
                    import re
                    match = re.search(r'mongodb\+srv://([^:]+):([^@]+)@([^/]+)', self.uri)
                    if match:
                        user, password, cluster = match.groups()
                        # Parse cluster name (e.g., wctest.mcvqzgb.mongodb.net -> ac-tfbsx1e-shard-00-00.mcvqzgb.mongodb.net)
                        cluster_parts = cluster.split('.')
                        shard_host = f"ac-tfbsx1e-shard-00-00.{cluster_parts[1]}.mongodb.net"
                        direct_uri = f"mongodb://{user}:{password}@{shard_host}:27017/{self.db_name}?tls=true&authSource=admin&directConnection=true&readPreference=secondary"
                        
                        self._client = AsyncIOMotorClient(direct_uri, serverSelectionTimeoutMS=10000)
                        self._db = self._client[self.db_name]
                        # Just test read access, don't ping admin
                        await self._db.list_collection_names()
                        print(f"✓ Connected to MongoDB Atlas (secondary, read-only): {self.db_name}")
                        self._read_only = True
                        # Skip index creation on secondary
                        return self
    ...
    async def _create_indexes(self):
        ...
        # sandbox_computations (Vercel Sandbox execution results)
        await db.sandbox_computations.create_index("computation_id", unique=True)
        await db.sandbox_computations.create_index("created_at")
    ...
    # =========================================================================
    # Sandbox Computations (Vercel Sandbox)
    # =========================================================================

    async def store_sandbox_result(
        self,
        computation_id: str,
        code: str,
        result: dict[str, Any],
        ...
        """Store a Vercel Sandbox execution result.

        Args:
            computation_id: Unique identifier for this computation
            code: The Python code that was executed
            result: SandboxResult dict with status, stdout, stderr, etc.
            config: Optional SandboxConfig dict (timeout, memory_mb, etc.)
        """
        await self._db.sandbox_computations.update_one(
    ...
    async def get_sandbox_result(self, computation_id: str) -> dict | None:
        """Get a sandbox computation result."""
        return await self._db.sandbox_computations.find_one({"computation_id": computation_id})

    async def list_sandbox_computations(
        self,
        status: str | None = None,
        limit: int = 50,
    ) -> list[dict]:
        """List recent sandbox computations, optionally filtered by status."""
        query: dict[str, Any] = {}
        if status:
            query["result.status"] = status

        cursor = self._db.sandbox_computations.find(query).sort("created_at", -1).limit(limit)
        return [doc async for doc in cursor]

    async def create_computation_id(self) -> str:
        """Generate a unique computation ID."""
        return f"sbx-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{str(uuid4())[:8]}"
